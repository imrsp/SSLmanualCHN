...
本版本将手册内容同步至 SSL Live 源站修订号 21.0.6，更新 Path Linking 与 KLANG 章节，并完善新版源站内容的审计兼容与回归检查。

#### Added

- **SSL Live 21.0.6 上游快照** — 保存新版官方源站转储及相对 21.0.4 的差异记录，新增 KLANG 映射通道和联动组自动化筛选示意图

#### Changed

- **Path Linking 内容更新** — 新增联动组成员关系、名称、颜色和参数筛选随场景变化的自动化说明，补充 Store/Recall Filter 配置步骤，并调整 Fast Setup 与联动组相关图片布局
- **KLANG 内容更新** — 新增 KLANG:app 的 Enable Mapped Channels 功能说明和示意图，并修正多张设置图片的显示尺寸与内容顺序
- **上游合并版本提升** — 将项目已完整合并并复核的源站修订号由 `21.0.4` 更新为 `21.0.6`

#### Fixed

- **内容审计兼容性** — 兼容新版源站导航标签的换行格式，避免源站正文指标被错误截断，并补充新内容与发布资源的回归测试
